...
export class EndlessMedicalCheckerService {
	...
	public initSession$(): Promise<ISession> {
		return firstValueFrom(
			this.http
				.get<ISession>('https://api-prod.endlessmedical.com/v1/dx/initSession')
				.pipe(
	...
	public acceptTerms(): Promise<void> {
		return firstValueFrom(
			this.http.post<void>(
				`${this.url}AcceptTermsOfUse?passphrase=${TERMS_PASSPHRASE}`,
				{}
			)
		);
	}
	...
	public updateSymptom(name: string, value: number): Promise<void> {
		return firstValueFrom(
			this.http.post<void>(
				`${this.url}UpdateFeature?name=${name}&value=${value}`,
				{}
			)
		);
	}
	...
	public suggestSymptoms$(): Observable<any> {
		return this.http
			.get<any>(
				`${this.url}GetSuggestedFeatures_PatientProvided?TopDiseasesToTake=10`
			)
			.pipe(
	...
	public getAllSymptoms$(): Observable<ISymptomWithDetails[]> {
		...
		return this.http
			.get<ISymptomWithDetails[]>(
				'https://endlessmedical.com/wp-content/uploads/SymptomChecker/assets/SymptomsOutput.json'
			)
			.pipe(
				map((allSymptoms: ISymptomWithDetails[]) => {
	...
	public getSameCategoryQuestion(
		category: string,
		alreadyAnsweredQuestions: string[]
	): ISymptomWithDetails | null {
		if (!this._allSymptoms.length) {
			return null;
		}

		const categorySymptoms = this._allSymptoms.filter(
			(symptom: ISymptomWithDetails) => symptom.category === category
		);
	...
	public analyze$(): Observable<IAnalysisResponse> {
		return this.http.get<IAnalysisResponse>(
			`${this.url}Analyze?NumberOfResults=5&ResponseFormat=full`
		);
	}
	...
	public getDocumentation(): Promise<IDocumentation> {
		return firstValueFrom(
			this.http
				.get<IDocumentationResponse>(
					`${this.url}GetMedicalDocumentation?SessionID=${this.sessionId}&format=json`
				)
				.pipe(map((response) => response.MedicalDocumentation))
		);
	}
